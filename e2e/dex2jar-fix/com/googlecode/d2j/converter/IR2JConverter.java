    ...
    private void reBuildJumpInstructions(IfStmt st, MethodVisitor asm) {
        ...
        default:
            ...
            if (isZeroOrNull(v1) || isZeroOrNull(v2)) { // IFx
                // PATCH (phonetemp e2e): when the zero is the FIRST operand (`0 > x`), the comparison
                // must be mirrored (`x < 0`). Upstream emitted IFGT for `if-gt 0, x`, inverting loops.
                boolean mirror = !isZeroOrNull(v2);
                if (!mirror) { // v2 is zero
                    accept(v1, asm);
                } else {
                    accept(v2, asm);
                }
                ...
                case EQ:
                    ...
                    break;
                case GE:
                    asm.visitJumpInsn(mirror ? IFLE : IFGE, target);
                    break;
                case GT:
                    asm.visitJumpInsn(mirror ? IFLT : IFGT, target);
                    break;
                case LE:
                    asm.visitJumpInsn(mirror ? IFGE : IFLE, target);
                    break;
                case LT:
                    asm.visitJumpInsn(mirror ? IFGT : IFLT, target);
                    break;
                default:
                    break;
                }
